...
@Component({
  selector: 'app-monthwise-material-consumption-report',
  templateUrl: './monthwise-material-consumption-report.component.html',
  styleUrls: ['./monthwise-material-consumption-report.component.css']
})
export class MonthwiseMaterialConsumptionReportComponent implements OnInit {
  ...
  getReport(print: Boolean) {
    ...
    if (this.queryForm.valid) {
      if (this._service.printExcelChk(print, this.queryForm.get('exportType')?.value)) {
        ...
        let payLoad: Object = {
          name: "BldgMatwiseconsumption.rpt",
          seqId: 1,
          ...
          reportParameters: {
            ...
            Remarks: this.queryForm.get('Remarks')?.value.length
              ? `'${this.queryForm.get('Remarks')?.value}'`
              : `''`,
