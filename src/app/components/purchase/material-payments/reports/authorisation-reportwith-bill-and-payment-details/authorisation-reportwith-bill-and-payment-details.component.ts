...
@Component({
  selector: 'app-authorisation-reportwith-bill-and-payment-details',
  templateUrl: './authorisation-reportwith-bill-and-payment-details.component.html',
  styleUrls: ['./authorisation-reportwith-bill-and-payment-details.component.css']
})
export class AuthorisationReportwithBillAndPaymentDetailsComponent implements OnInit {
  ...
  queryForm: FormGroup = new FormGroup({
    ...
    name: new FormControl<string>('withOutHsn', Validators.required),
    range: new FormGroup({
  ...
  getReport(print: Boolean) {
    console.log(this.queryForm.get('name')?.value, "name");
    if (this.queryForm?.valid) {
      if (this._service.printExcelChk(print, this.queryForm.get('exportType')?.value)) {
        this.loaderToggle = true;
        let payload: Object = {
          name: this.queryForm.get('name')?.value == 'withOutHsn' ? "AuthorisationBillsPayments.rpt" : "AuthorisationBillsPaymentshsn.rpt",
          isPrint: false,
          seqId: 1,
