...
export class PfreportComponent implements OnInit {
  ...
  pfReportMonth: FormGroup = new FormGroup({
    name: new FormControl('PYRL_PFReport.rpt'),
  ...
  getReport(print: boolean) {
    console.log("fromvalue",this.pfReportMonth.value);    
    if (this.pfReportMonth.valid) {
  ...
  setReportValue() {
    this.payLoad = {
      name: 'PYRL_PFReport.rpt',
      isPrint: false,
      reportParameters: {
        PCoyFrom: `'${this.pfReportMonth.controls['reportParameters']?.get('PCoyFrom')?.value[0][0]}'`,
        PCoyTo: `'${this.pfReportMonth.controls['reportParameters']?.get('PCoyTo')?.value[0][0]}'`,
        PPayMonthFrom: `'${this.pfReportMonth.controls['reportParameters']?.get('PPayMonthFrom')?.value}'`,
        PSalType: `'${this.pfReportMonth.controls['reportParameters']?.get('PSalType')?.value?.join("','")}'`
      }
    } ;
  }
