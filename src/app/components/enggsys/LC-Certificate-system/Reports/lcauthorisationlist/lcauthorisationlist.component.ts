...
@Component({
  selector: 'app-lcauthorisationlist',
  templateUrl: './lcauthorisationlist.component.html',
  styleUrls: ['./lcauthorisationlist.component.css']
})
export class LcauthorisationlistComponent implements OnInit {
  ...
  Lcauthorisationlist: FormGroup = new FormGroup(
    {
      name: new FormControl('Engg_RP_LCAuth_List.rpt'),
  ...
  ngOnInit(): void {
    ...
    this.setFocus('Lcauthorisationlist_bldg');
  }
  ...
  buttonAction(event: string){
    ...
    if (event == 'reset') {
      this.Lcauthorisationlist.reset();
      this.setFocus('Lcauthorisationlist_bldg');
    }
    if (event == 'exit') {
  ...
  getReport(print: boolean) {
    console.log('fromvalue', this.Lcauthorisationlist.value);
    if (this.Lcauthorisationlist.valid) {
      ...
      this.commonReportService
        ...
        .subscribe({
          next: (res: any) => {
            ...
            this._service.exportReport(print, res, this.Lcauthorisationlist.controls['reportParameters'].get('exportType')?.value ,fileName)
          },
          error: (err: any) => {
            this.loaderToggle = false;
          },
  ...
  setReportValue() {
    this.payLoad = {
      name: 'Engg_RP_LCAuth_List.rpt',
      isPrint: false,
      exportType : this.Lcauthorisationlist.controls['reportParameters'].get('exportType')?.value,
      reportParameters: {
        bldgCode: this.Lcauthorisationlist.controls['reportParameters']?.get(
          'bldgCode'
        )?.value
          ? "'" +
            this.commonService.convertArryaToString(
              this.Lcauthorisationlist.controls['reportParameters']?.get(
                'bldgCode'
              )?.value
            ) +
            "'"
        ...
        PartyCode: this.Lcauthorisationlist.controls['reportParameters']?.get(
          'PartyCode'
        )?.value
          ? "'" +
            this.commonService.convertArryaToString(
            ...
            "'"
          : `'ALL'`,
        MatGroup: this.Lcauthorisationlist.controls['reportParameters']?.get(
          'MatGroup'
        )?.value
          ? "'" +
            this.commonService.convertArryaToString(
        ...
        FromDate:
          "'" +
          moment(
            this.Lcauthorisationlist.getRawValue().reportParameters.FromDate
          ).format('DD/MM/YYYY') +
          "'",
        UptoDate:
          "'" +
          moment(
            this.Lcauthorisationlist.getRawValue().reportParameters.UptoDate
          ).format('DD/MM/YYYY') +
          "'",
      },
    };
  ...
  GetDefaultValue() {
    ...
    this.payreportsService.GetReportParameters().subscribe((res: any) => {
      if (res.status) {
        ...
        this.Lcauthorisationlist.patchValue({
          reportParameters: {
